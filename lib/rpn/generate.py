...
from fast_rcnn.train import filter_roidb, get_training_roidb
...
def imdb_proposals(net, imdb):
    ...
    for i in xrange(imdb.num_images):
        ...
        if 0:
            dets = np.hstack((imdb_boxes[i], imdb_scores[i]))
...
def rpn_evaluate(net, imdb, IoUThresh=0.5, numMaxDets=[200,500,1000,2000], rpn_file=None, score_file=None):
