...
//  LOG(INFO) << "[WANGHAN DEBUG] bottom0_width=" << bottom[0]->width() << " scale_w_=" << scale_w_ << " upsample_w_=" << upsample_w_ << " top0_width=" << top[0]->width() << " top0_shape3=" << top[0]->shape(3);
}
...
//  const Dtype* bottom_mask_data = bottom[1]->cpu_data();
  ...
  for (int n = 0; n < bottom[0]->num(); ++n) {
    for (int c = 0; c < channels_; ++c) {
      for (int j = 0; j < upsample_h_; ++j) {
        for(int i = 0; i < upsample_w_; ++i) {
          // int idx = j * scale_h_ * upsample_h_ + i * scale_w_;
          int idx_i = i / scale_w_;
          int idx_j = j / scale_h_;
          top_data[j*upsample_w_+i] = bottom_data[idx_j*width_+idx_i];  
      ...
      for (int c = 0; c < channels_; ++c) {
        for (int j = 0; j < upsample_h_; ++j) {
          for(int i = 0; i < upsample_w_; ++i) {
            int idx_i = i / scale_w_;
            int idx_j = j / scale_h_;
            bottom_diff[idx_j*width_+idx_i] += top_diff[j*upsample_w_+i];  
          }
        }
        // compute offset
